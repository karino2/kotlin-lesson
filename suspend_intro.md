...
2. 謎の誰かがこの「別のスレッド」で何か処理をして、呼び出した関数から抜けたあとに、あとからコールバックで結果が返ってくる
...
launchの中は、その書かれたスレッドで実行されて、べつだん新たなスレッドが作られる訳じゃない、というのは大切なポイントです。
...
ただ、普通のコールバックとは違ってsuspend関数の結果の呼び出しは「直接はコードの上では見えない」というのがちょっと違うところで、ここがsuspend関数がちょっと難しく感じるところです。
